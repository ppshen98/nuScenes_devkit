...
- python export_kitti.py nuscenes_tracking_gt_to_kitti --nusc_kitti_dir ~/nusc_kitti
...
class KittiConverter:
    def __init__(self,
        ...
        self.nusc = NuScenes(version=nusc_version, dataroot='/media/shen/系统/ppshen/Dataset/Nuscenes/Mini/v1.0-mini')
    ...
    def nuscenes_tracking_gt_to_kitti(self) -> None:
        """
        Converts nuScenes GT annotations to KITTI format.
        """
        kitti_to_nu_lidar = Quaternion(axis=(0, 0, 1), angle=np.pi / 2)
        ...
        for sample_scene in sample_scenes:
            ...
            # tokens = []
            labels = ''
            for token_idx, sample_token in enumerate(frame_tokens):
                ...
                # Convert image (jpg to png).
                src_im_path = os.path.join(self.nusc.dataroot, filename_cam_full)
                sub_image_folder = os.path.join(image_folder, sample_scene['name'])
                if not os.path.isdir(sub_image_folder):
                    os.makedirs(sub_image_folder)
                dst_im_path = os.path.join(sub_image_folder, '%06d' % token_idx + '.png')
                if not os.path.exists(dst_im_path):
                    # print(src_im_path, dst_im_path)
                    im = Image.open(src_im_path)
                    im.save(dst_im_path, "PNG")
                ...
                if not self.camera_only:
                    src_lid_path = os.path.join(self.nusc.dataroot, filename_lid_full)
                    sub_lid_folder = os.path.join(lidar_folder, sample_scene['name'])
                    if not os.path.isdir(sub_lid_folder):
                        os.makedirs(sub_lid_folder)
                    dst_lid_path = os.path.join(sub_lid_folder, sample_scene['name'] + '_' + '%06d' % token_idx + '.bin')
                    assert not dst_lid_path.endswith('.pcd.bin')
                    pcl = LidarPointCloud.from_file(src_lid_path)
                    pcl.rotate(kitti_to_nu_lidar_inv.rotation_matrix)  # In KITTI lidar frame.
                    with open(dst_lid_path, "w") as lid_file:
                ...
                # Create calibration file.
                calib_path = os.path.join(calib_folder, sample_scene['name'] + '.txt')
                ...
                for sample_annotation_token in sample_annotation_tokens:
                    ...
                    if instance_token in Instance_to_ID:
                        ID = Instance_to_ID[instance_token]
                    else:
                        ID = self.ID
                        Instance_to_ID.update({instance_token: self.ID})
                        self.ID += 1
                    ...
                    # Convert box to output string format.
                    labels += '{} {} '.format(frame, ID) + KittiDB.box_to_string(name=detection_name, box=box_cam_kitti,
                                                                                bbox_2d=bbox_2d, truncation=truncated,
                                                                                occlusion=occluded) + '\n'

            # Write label file.
            label_path = os.path.join(label_folder, sample_scene['name'] + '.txt')
            if os.path.exists(label_path):
                print('Skipping existing file: %s' % label_path)
                continue
            else:
    ...
    def _parse_frame_tokens(self, scene) -> List[str]:
        frame_tokens: List[str] = []
        frame_token = scene['first_sample_token']  # first frame token
        ...
        expected_num_frames = scene["nbr_samples"]
